...
class ATT(nn.Module):
    ...
    def forward(self, x):
        ...
        out_H = torch.bmm(proj_value_H.float(), att_H.permute(0, 2, 1)).view(m_batchsize, width, -1, height).permute(0, 2, 3, 1)
        out_W = torch.bmm(proj_value_W.float(), att_W.permute(0, 2, 1)).view(m_batchsize, height, -1, width).permute(0, 2, 1, 3)
        # print(out_H.size(),out_W.size())

        out_C = proj_key_C * proj_value
        return self.gamma * (out_H + out_W + out_C) + x

##  cross-dimension feature fusion module
class FFM(nn.Module):
    def __init__(self, dim):
        ...
        # conv1x1
        self.conv1x1_out = Conv(c1=2*dim, c2=dim, k=1, s=1, p=0, g=1, act=True)
    ...
    def forward(self, x):
        ...
        # Fusion module
        concat_fea = self.concat([rgb_fea, ir_fea])
        new_fea = self.att(concat_fea)
        fused_fea = self.conv1x1_out(new_fea)
...
# Dual Feature Enhancement Module  2024/10/31 added by wwc
class DAM(nn.Module):
    def __init__(self, in_channels):
        super(DAM, self).__init__()
        self.maxpool = AdaptivePool2d(1, 1, 'max')
        self.convCx1 = Conv(in_channels, 1, 1, 1)
        self.convCxHC = Conv(in_channels, in_channels//2, 1, 1)
        self.convHCxHC = Conv(in_channels//2, in_channels//2, 1, 1)
        #self.convHCxC = Conv_withoutBN(in_channels//2, in_channels, 1, 1)
        self.convHCxC = nn.Conv2d(in_channels // 2, in_channels, 1, bias=False)
    ...
    def forward(self, x):
        ...
        rgb_fea0, rgb_fea1 = rgb_fea.chunk(2, dim=1) # B, C/2, H, W；   B, C/2, H, W
        ir_fea0, ir_fea1 = ir_fea.chunk(2, dim=1) # B, C/2, H, W；  B, C/2, H, W

        # channel-interaction
        rgb_fea_conv = self.convHCxHC(rgb_fea0).contiguous().view(B, C//2, -1)  # B, C/2, HW

        ir_fea_conv = self.convHCxHC(ir_fea0).contiguous().view(B, C//2, -1)  # B, C/2, HW

        fea_cat0 = torch.cat([rgb_fea1, ir_fea1], dim=1)  # B, C, H, W
        fea_catC1 = self.convCx1(fea_cat0) # B, 1, H, W
        fea_catC = fea_catC1.contiguous().view(B, 1, -1).permute(0, 2, 1).softmax(1) # B, HW, 1

        rgb_fea_C = torch.matmul(rgb_fea_conv, fea_catC).contiguous().view(B, C//2, 1, 1) # B, C/2, 1, 1
        rgb_fea_C = self.layernorm(self.convHCxC(rgb_fea_C)).sigmoid() # B, C, 1, 1
        #rgb_fea_C = self.convHCxC(rgb_fea_C).sigmoid() # B, C, 1, 1

        ir_fea_C = torch.matmul(ir_fea_conv, fea_catC).contiguous().view(B, C//2, 1, 1) # B, C/2, 1, 1
        ir_fea_C = self.layernorm(self.convHCxC(ir_fea_C)).sigmoid()  # B, C, 1, 1
        #ir_fea_C = self.convHCxC(ir_fea_C).sigmoid()  # B, C, 1, 1

        rgb_fea_C_product = rgb_fea * rgb_fea_C # B, C, H, W
        ir_fea_C_product = ir_fea * ir_fea_C # B, C, H, W

        rgb_fea_channel = rgb_fea + ir_fea_C_product # B, C, H, W
        ir_fea_channel = ir_fea + rgb_fea_C_product # B, C, H, W

        # spatial-interaction
        fea_catS1 = self.convCxHC(fea_cat0) # B, C/2, H, W
        fea_catS = self.maxpool(fea_catS1).contiguous().view(B, C//2, 1).permute(0, 2, 1).softmax(2) # B, 1, C/2

        rgb_fea_S = torch.matmul(fea_catS, rgb_fea_conv).contiguous().view(B, 1, H, W)  # B, 1, H, W
        rgb_fea_S = rgb_fea_S.sigmoid()  # B, 1, H, W

        ir_fea_S = torch.matmul(fea_catS, ir_fea_conv).contiguous().view(B, 1, H, W)  # B, 1, H, W
        ir_fea_S = ir_fea_S.sigmoid()  # B, 1, H, W

        rgb_fea_S_product = rgb_fea * rgb_fea_S  # B, C, H, W
        ir_fea_S_product = ir_fea * ir_fea_S  # B, C, H, W

        rgb_fea_spatial = rgb_fea + ir_fea_S_product  # B, C, H, W
        ir_fea_spatial = ir_fea + rgb_fea_S_product  # B, C, H, W

        new_rgb_fea = rgb_fea_channel + rgb_fea_spatial # B, C, H, W
        new_ir_fea = ir_fea_channel + ir_fea_spatial # B, C, H, W

        # Channel Shuffle
        rgb_fea_shu = channel_shuffle(new_rgb_fea, 32) # B, C, H, W
        ir_fea_shu = channel_shuffle(new_ir_fea,  32) # B, C, H, W

        rgb_fea_pool = self.maxpool(rgb_fea_shu).sigmoid() # B, 1, H, W
        ir_fea_pool = self.maxpool(ir_fea_shu).sigmoid() # B, 1, H, W

        out_rgb_fea = new_rgb_fea * rgb_fea_pool # B, C, H, W
        out_ir_fea = new_ir_fea * ir_fea_pool # B, C, H, W

        out_rgb_fea = out_rgb_fea + rgb_fea # B, C, H, W
        out_ir_fea = out_ir_fea + ir_fea # B, C, H, W

        return [out_rgb_fea, out_ir_fea]