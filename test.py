...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if plots and batch_i < 16:
            f_label_rgb = save_dir / f'test_batch{batch_i}_labels_rgb.jpg'  # labels
            Thread(target=plot_images, args=(img_rgb, targets, paths, f_label_rgb, names), daemon=True).start()
            f_pred_rgb = save_dir / f'test_batch{batch_i}_pred_rgb.jpg'  # predictions
            Thread(target=plot_images, args=(img_rgb, output_to_target(out), paths, f_pred_rgb, names), daemon=True).start()

            f_label_ir = save_dir / f'test_batch{batch_i}_labels_ir.jpeg'  # labels
            Thread(target=plot_images, args=(img_ir, targets, paths, f_label_ir, names), daemon=True).start()
            f_pred_ir = save_dir / f'test_batch{batch_i}_pred_ir.jpeg'  # predictions
            Thread(target=plot_images, args=(img_ir, output_to_target(out), paths, f_pred_ir, names), daemon=True).start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./weights/CDFNet_M3FD.pt', help='model.pt path(s)')
    ...
    p = "/home/F/wwc/PythonProjects/CDFNet/dataset/M3FD_Detection/labels/test"
